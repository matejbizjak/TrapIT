...
public class DB_Creator_Temporary {
	public static void main(String[] args) {
		...
		try {
			...
			PrintWriter outputFile = new PrintWriter("Scripts/TrapIT_Setup_Temp.sql", "UTF-8");
            PrintWriter mediaFiller = new PrintWriter("Scripts/Media_Fill_Temp.sql", "UTF-8");
			...
			if(args.length == 1) {
				...
				if(args[0].equals("-db")) {
					...
					scriptToDB(myDBConnection, "Scripts/TrapIT_Setup_Temp.sql");
	...
	public static void mediaToScript(String fName, PrintWriter outFile) {
		...
		while(inFile.hasNext()) {
			...
			String sql = 	"INSERT INTO media (`date`, `name`, `empty`, `image`, `interesting`, `comment`, `site_id`, `path_id`)" + 
								"VALUES (\""+ lineElements[0] + "\", \"" + lineElements[7].substring(24) + lineElements[1] + "\", " + lineElements[2] + ", " + lineElements[3] + ", " + lineElements[4] + ", \"" + lineElements[5] + "\", (" + 
								"	SELECT site_id " + 
								"	FROM site " + 
								...
								"	SELECT path_id " + 
								"	FROM path " + 
								"	WHERE value=\"" + lineElements[7].substring(0, 24) + "\"" + 
								"));";
